...
exports.uploadFostacDoc = multer({
    storage: multerS3({
        ...
        key: function (req, file, cb) {
            cb(null, `${fostacDocPath}${(Date.now().toString() + '-' + file.originalname.split(' ').join(''))}`);
        }
    })
});

exports.uploadFoscosDoc = multer({
    storage: multerS3({
        s3: s3Client,
        bucket: AWS_S3.bucket,
        metadata: function (req, file, cb) {
        ...
        key: function (req, file, cb) {
            cb(null, `${foscosDocPath}${(Date.now().toString() + '-' + file.originalname.split(' ').join(''))}`);
        }
    })
});

exports.uploadHraDoc = multer({
    storage: multerS3({
        s3: s3Client,
        bucket: AWS_S3.bucket,
        metadata: function (req, file, cb) {
...
exports.uploadCheque = multer({
    storage: multerS3({
        ...
        key: function (req, file, cb) {
            cb(null, `${chequePath}${(Date.now().toString() + '-' + file.originalname.split(' ').join(''))}`);
        }
    })
});
